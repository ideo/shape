  ...
  describe '#notify' do
    ...
    it 'renders details' do
      ...
      expect(mail.body.encoded).to include(
        "Please add a payment method to continue using Shape at #{organization.name}."\
        " The #{organization.name} instance of Shape will be frozen on"\
        " #{(organization.overdue_at + 2.weeks).to_s(:mdy)} if you do not add a valid payment method.",
      )
