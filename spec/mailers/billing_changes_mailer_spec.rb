...
RSpec.describe BillingChangesMailer, type: :mailer do
  ...
  describe '#notify' do
    ...
    it 'renders details about billing changes' do
      ...
      expect(mail.body.encoded).to match("New users: #{new_active_users}")
      expect(mail.body.encoded).to match("Total users: #{organization.active_users_count}")
      expect(mail.body.encoded).to match("Additional Monthly Charge: #{format_currency(Organization::PRICE_PER_USER * new_active_users)}")
      expect(mail.body.encoded).to match("New total monthly charge: #{format_currency(Organization::PRICE_PER_USER * organization.active_users_count)}")
      ppu = Organization::PRICE_PER_USER / ((Time.current.end_of_month.day - Time.current.day) + 1)
      expect(mail.body.encoded).to match("Proprated charge this month: #{format_currency(new_active_users * ppu)}")
      expect(mail.body.encoded).to match("Next statement date: #{Time.now.utc.end_of_month.to_s(:mdy)}")
      expect(mail.body.encoded).to match("Go To Shape: #{root_url}")
    end
  end
end
