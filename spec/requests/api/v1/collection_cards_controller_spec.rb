...
describe Api::V1::CollectionCardsController, type: :request, json: true, auth: true do
  ...
  describe 'GET #index' do
    ...
    context 'with pagination options' do
      let!(:collection) { create(:collection, num_columns: nil, num_cards: 12, add_editors: [user]) }
      let(:path) { "/api/v1/collections/#{collection.id}/collection_cards?per_page=10&page=2" }

      it 'includes only the correct page of collection cards' do
        get(path)
        # should be the second page (2 cards)
        expect(json['data'].map { |cc| cc['id'].to_i }).to match_array(
          collection.collection_card_ids.slice(10, 2),
        )
      end

      context 'with hidden = true option on a board' do
        let!(:collection) { create(:board_collection, num_cards: 12, add_editors: [user]) }
        let(:path) { "/api/v1/collections/#{collection.id}/collection_cards?per_page=10&page=2&hidden=true" }
        before do
          collection.collection_cards.last(10).each { |cc| cc.update(hidden: true) }
        end

        it 'includes only the correct page of collection cards, showing hidden cards' do
          get(path)
          # should be the second page (2 cards)
          expect(json['data'].map { |cc| cc['id'].to_i }).to match_array(
            collection.collection_card_ids.slice(10, 2),
