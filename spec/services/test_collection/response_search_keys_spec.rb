...
RSpec.describe TestCollection::ResponseSearchKeys, type: :service do
  ...
  let(:expected_keys) do
    ...
    graphable_answers.map do |answer|
      if answer.question_choices_customizable?
        ...
        org_question_type = answer.question.question_id
      else
      ...
      answer_vals.map do |val|
        ...
        keys << "organization_#{test_collection.organization_id}_question_#{org_question_type}_answer_#{val}"
      end
    end
    keys
  end
  ...
  describe '#call' do
    it 'returns keys for all graphable questions' do
      expect(subject).to eq(expected_keys.sort)
    end
  end
end
