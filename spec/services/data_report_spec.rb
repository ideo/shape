...
RSpec.describe DataReport, type: :service do
  ...
  describe '#call' do
    ...
    xcontext 'filtering by collection' do
      ...
      before do
        create_list(:activity, 1, target: parent_collection, action: :created)
        create_list(:activity, 5, target: parent_collection, action: :viewed)
        create_list(:activity, 2, target: child_collection, action: :edited)
      ...
      context 'with a participant measure' do
        it 'calculates the number of participants in the collection, child collections, and items in those collections' do
          ...
          expect(report.call[:value]).to eq 10
        end
