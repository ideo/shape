...
RSpec.describe OrganizationAutojoiner, type: :service do
  ...
  describe '#available_orgs' do
    ...
    context 'with autojoinable group' do
      let!(:group) { create(:group, organization: organization, autojoin_emails: ['specific@email.com']) }
      let(:user) { create(:user, email: 'specific@email.com') }

      it 'should return the group org' do
        orgs = OrganizationAutojoiner.new(user).available_orgs
        expect(orgs).to match_array [organization]
      end
    end
  end
  ...
  describe '#autojoin' do
    ...
    context 'with autojoinable group' do
      ...
      it 'should autojoin the group' do
        joined = OrganizationAutojoiner.new(user).autojoin
