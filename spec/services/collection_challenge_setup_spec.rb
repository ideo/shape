...
RSpec.describe CollectionChallengeSetup, type: :service do
  ...
  let(:collection) { create(:collection, organization: organization, collection_type: 'challenge', created_by: current_user) }
  let(:challenge_setup) { CollectionChallengeSetup.new(collection: collection, current_user: current_user) }
