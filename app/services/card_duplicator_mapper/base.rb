...
module CardDuplicatorMapper
  class Base
    attr_accessor :duplicated_cards_hash, :linked_cards_hash

    def initialize(batch_id:)
      @batch_id = batch_id
      @duplicated_cards_hash = Redis::HashKey.new('#{@batch_id}_duplicated_cards', expireat: lambda { Time.now + 1.day })
      @linked_cards_hash = Redis::HashKey.new('#{@batch_id}_linked_cards', expireat: lambda { Time.now + 1.day })
    end
    ...
    def register_duplicated_card(original_card_id:, to_card_id:)
      ...
      @duplicated_cards_hash[original_card_id] = to_card_id
    ...
    def register_linked_card(card_id:, data:)
      ...
      #   data.to_hash,
      # )
      @linked_cards_hash[card_id] = data.to_hash
    end

    def expire_data_in_one_day!
      # FIXME: any reason we'd need to expire over here?
      # Cache.expire("#{@batch_id}_linked_cards", 1.day.to_i)
      # Cache.expire("#{@batch_id}_duplicated_cards", 1.day.to_i)
    end

    def clear_cached_data!
      @duplicated_cards_hash.delete
      @linked_cards_hash.delete
    end
  end
end
